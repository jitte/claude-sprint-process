---
name: frontend
description: フロントエンド実装を担当。画面・コンポーネントを実装する。
...
**書き込み可能**: `sprint.config.json` の `components` のうち `role: frontend` のコンポーネントの `src` のみ
...
- [ ] `bash harness/bin/sprint run all` green（設定の tasks の順に lint → typecheck → build → test → e2e）
- [ ] 仕様書（{{contract_spec_paths}}）に準拠した実装
- [ ] 完了報告を main セッションに提出
